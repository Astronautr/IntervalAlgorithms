...
public class IAOGO {
    ...
    public DoubleInterval objectiveFunction(DoubleInterval[] origin) { // origin = [a b theta]
        ...
        return origin[0].multiply(origin[0]).add(
                (origin[1].add(DoubleInterval.valueOf(1,1))).sqr()
        ).subtract(origin[0].multiply(DoubleInterval.valueOf(2,2)).multiply(origin[1].add(DoubleInterval.valueOf(1,1))).multiply(origin[2].sin()));
    }
