...
public class Gradient {
    ...
    public Gradient pow(Gradient Y) {
        ...
        result.dX = new SetInterval[dim];
        for (int i = 0; i < dim; i++) {
            if(Y.dX[i].equal(ZERO))
                result.dX[i] = ic.mul(ic.mul(Y.X, ic.pow(X,ic.sub(Y.X, ic.numsToInterval(1,1)))),this.dX[i]);
            else
                if (this.dX[i].equal(ZERO))
                    result.dX[i] = ic.mul(ic.mul(ic.log(this.X),ic.pow(this.X,Y.X)),Y.dX[i]);
                else
                    result.dX[i] = ic.mul(ic.pow(this.X,ic.sub(Y.X,ONE)),ic.add(ic.mul(this.dX[i],Y.X),ic.mul(ic.mul(this.X,ic.log(this.X)),Y.dX[i])));
        }
        return result;
    }
